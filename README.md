...
署名（DEVELOPMENT_TEAM）は `project.yml` に設定済み。**別のチーム/アカウントで動かす場合**は
`project.yml` の `DEVELOPMENT_TEAM`・Bundle ID（`com.mtkg.LearnLanguage`）と、後述の
App Group（`group.com.mtkg.LearnLanguage`）・iCloud コンテナ（`iCloud.com.mtkg.LearnLanguage`）を
自分の識別子に読み替えること（entitlements 2 ファイルと `SharedInbox.appGroupID` にも登場する）。
