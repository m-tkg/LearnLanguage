...
6. **設定項目（`@AppStorage`）を追加したら `SettingsCloudSync.syncedKeys` にも追加する。**
   一般設定は iCloud KVS で端末間同期される（UserDefaults ⇄ KVS のミラーリング、衝突時はクラウド優先）。
   キー一覧への追加を忘れると、その項目だけ同期されない。API キー等の秘密情報は KVS に載せず、
   `KeychainStore` の同期属性（iCloud キーチェーン）に任せる。

7. **モック/テストダブルは使う側の近くに置く。** Services 層に「誰にも呼ばれないモック」を
